# Sonatype Resource Allocation Automation
...
│   ├── organizations.json      # Mapping of Chinese names to IQ Server Organization IDs
│   └── package_manager.json    # Configuration for supported package managers
├── logs/                       # Log files will be generated here
└── src/
    └── resource_allocation/
        ├── api.py              # FastAPI application & endpoints
        ├── clients.py          # API clients & specialized business logic managers
...
- FastAPI endpoints for repository operations
- Request validation and authentication
- Batch processing coordination

**Business Logic Layer** (`clients.py`):

- **API Clients**: `NexusClient`, `IQServerClient` - Handle HTTP communication
- **Specialized Managers**:
  - `ResourceCreator` - Creates repositories, privileges, and roles
  - `UserRoleManager` - Manages user role assignments
  - `ResourceCleaner` - Handles resource cleanup and deletion
  - `IQServerManager` - Manages IQ Server role assignments
- **Orchestrator**: `PrivilegeManager` - Coordinates operations using specialized managers

**Configuration Layer** (`config.py`):

- Configuration providers following dependency injection pattern
- Data classes for type safety and immutability

**Utilities** (`common.py`):

- Custom exceptions, logging configuration, utility functions
...
- **Single Responsibility**: Each class has one clear purpose
  ...
  - `ResourceCreator` only handles resource creation
  - `UserRoleManager` only handles user role assignments
  - `ResourceCleaner` only handles cleanup operations
  - `IQServerManager` only handles IQ Server operations

- **Open/Closed**: Easy to extend without modifying existing code

  - New package managers can be added via configuration
  - New operation types can be added by extending the factory pattern

- **Dependency Inversion**: High-level modules depend on abstractions

  - `PrivilegeManager` orchestrates operations without knowing implementation details
  - Configuration providers are injected as dependencies
...
- **Enhanced Reusability**: Managers can be reused in different contexts
- **Clearer Dependencies**: What each component needs is explicit
- **Easier Debugging**: Logs clearly indicate which manager is performing operations
...
| `DEBUG`             | If `true`, the service uses `organizations-debug.json` instead of `organizations.json`. | `false`                            |
...
3. Execute the binary:
   ...
   - **Windows:**
     ```cmd
     sonatype-resource-allocation-automation.exe
     ```
...
_The `id` field is the IQ Server organization ID. The `name` field is used in API requests._
...
All endpoints require **HTTP Bearer Token** authentication.
...
- **Request Body:**
  ...
  ```json
  {
    "requests": [
      {
        "organization_name": "Application Service Development Dept",
      ...
      {
        "organization_name": "Treasury IT Dept",
...
| ------------------- | --------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `organization_name` | `string`  | Yes           | The name of the organization. Must match an entry in `config/organizations.json`.                                                                                                                                              |
...
- **Generated Names:**
  ...
  - **IQ Server**: Grants "Owner" role for the organization specified by `organization_name` to `ldap_username`.
...
{
  "success": true,
  "message": "Processed 1 of 2 requests successfully",
  "processed_count": 1,
  "total_requests": 2,
  "results": [
    {
...
- **`500 Organization '組織名稱' not found`**:
  - The `organization_name` in your request must precisely match a `name` entry in `config/organizations.json`. Check for typos or missing entries.
